...
public class PreprocessPipelineSeparate implements PreprocessPipeline {
    ...
    public Map<String, List<String>> preprocessRequirements(boolean compare, List<Requirement> requirements) throws InternalErrorException {
        ...
        try {
            for (Requirement requirement : requirements) {
                ...
                String name = extractRequirementName(compare, requirement);
                String text = extractRequirementText(compare,requirement);
                List<String> tokens_name = englishAnalyze(name);
                List<String> tokens_text = englishAnalyze(text);
                result.put(id,tokens_name);
                result.put(id+"__text__",tokens_text);
            }
        } catch (IOException e) {
    ...
    private String extractRequirementName(boolean compare, Requirement requirement) {
        String result = "";
        if (requirement.getName() != null) result = result.concat(cleanText(requirement.getName()) + ". ");
        return result;
    }

    private String extractRequirementText(boolean compare, Requirement requirement) {
        String result = "";
